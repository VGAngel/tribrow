 ...
 * </pre>
 */
public class RtsCamNew extends AbstractAppState {
     ...
     * @param accelTime amount of time which is need to accelerate to full speed in seconds (has to be bigger than zero, values over half second feel very sluggish). Defaults are 0.4
     *                  seconds
     */
     ...
     * @param accelTime        amount of time which is need to accelerate to full speed in seconds (has to be bigger than zero, values over half second feel very sluggish). Defaults are 0.4
     *                         seconds
     * @param decelerationTime amount of time in seconds which is needed to automatically decelerate (friction-like stopping) from maxSpd to full stop.
     */
     ...
     *
     * @param rotationButton button to hold to control TILT/ROTATION with mouse movements
    ...
    public static void applyCamera(Application application) {
        application.getStateManager().detach(application.getStateManager().getState(FlyCamAppState.class));
        ...
        application.getStateManager().attach(rtsCam);
