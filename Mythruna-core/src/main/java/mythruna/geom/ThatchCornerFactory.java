...
public class ThatchCornerFactory implements GeomFactory {
    ...
    public ThatchCornerFactory(int materialType, int innerMaterialType, int dir) {
    ...
    public boolean isSameShape(GeomFactory f) {
        ...
        if ((f == null) || (f.getClass() != getClass()))
            return false;
        ThatchCornerFactory other = (ThatchCornerFactory) f;
    ...
    public int createInternalGeometryPoly(GeomPartBuffer buffer, int x, int y, int z, int xWorld, int yWorld, int zWorld, float sun, float light, BlockType block) {
        ...
        Vector3f[] verts = {new Vector3f(innerMin.x, innerMax.y, innerMax.z), new Vector3f(innerMax.x, innerMin.y, innerMax.z), new Vector3f(innerMax.x, innerMin.y, innerMin.z), new Vector3f(innerMin.x, innerMin.y, innerMin.z), new Vector3f(outerMin.x, outerMax.y + tinyNest, outerMax.z + nest), new Vector3f(outerMax.x + nest, outerMin.y, outerMax.z - nest), new Vector3f(outerMax.x, outerMin.y, outerMin.z + nest), new Vector3f(outerMin.x, outerMin.y, outerMin.z + nest)};
        ...
        Vector3f outerNormal2 = new Vector3f(1.0F, 0.2F, 1.0F);
        innerNormal.normalizeLocal();
        outerNormal.normalizeLocal();
        outerNormal2.normalizeLocal();

        Vector3f[] normals = {new Vector3f(innerNormal2.x, innerNormal2.y, innerNormal2.z), new Vector3f(innerNormal.x, innerNormal.y, innerNormal.z), new Vector3f(0.2F, 1.0F, -0.2F).normalizeLocal(), new Vector3f(innerNormal.z, innerNormal.y, -innerNormal.x), new Vector3f(-outerNormal2.x, outerNormal2.y, outerNormal2.z), new Vector3f(outerNormal.x, outerNormal.y, outerNormal.z), new Vector3f(0.5F, -0.2F, -0.5F).normalizeLocal(), new Vector3f(-outerNormal.z, outerNormal.y, -outerNormal.x)};
        ...
        part.setCoords(new float[]{xCenter + verts[0].x, yCenter + verts[0].y, zCenter + verts[0].z, xCenter + verts[1].x, yCenter + verts[1].y, zCenter + verts[1].z, xCenter + verts[2].x, yCenter + verts[2].y, zCenter + verts[2].z, xCenter + verts[3].x, yCenter + verts[3].y, zCenter + verts[3].z, xCenter + verts[4].x, yCenter + verts[4].y, zCenter + verts[4].z, xCenter + verts[5].x, yCenter + verts[5].y, zCenter + verts[5].z, xCenter + verts[6].x, yCenter + verts[6].y, zCenter + verts[6].z, xCenter + verts[7].x, yCenter + verts[7].y, zCenter + verts[7].z});

        part.setNormals(new float[]{normals[0].x, normals[0].y, normals[0].z, normals[1].x, normals[1].y, normals[1].z, normals[2].x, normals[2].y, normals[2].z, normals[3].x, normals[3].y, normals[3].z, normals[4].x, normals[4].y, normals[4].z, normals[5].x, normals[5].y, normals[5].z, normals[6].x, normals[6].y, normals[6].z, normals[7].x, normals[7].y, normals[7].z});

        float textureOffset = 0.2F;
        part.setTexCoords(new float[]{0.5F - textureOffset, 0.5F - textureOffset, 0.5F + textureOffset, 0.5F - textureOffset, 0.5F + textureOffset, 0.5F + textureOffset, 0.5F - textureOffset, 0.5F + textureOffset, 0.0F, 0.0F, 1.0F, 0.0F, 1.0F, 1.0F, 0.0F, 1.0F});

        part.setIndexes(new short[]{0, 1, 2, 0, 2, 3, 0, 5, 1, 0, 4, 5, 1, 5, 2, 2, 5, 6, 6, 7, 3, 2, 6, 3, 0, 3, 4, 3, 7, 4});
        ...
        textureOffset = 0.3F;
        part.setTexCoords(new float[]{0.5F - textureOffset, 0.5F - textureOffset, 0.5F - textureOffset, 0.5F + textureOffset, 0.5F + textureOffset, 0.5F + textureOffset, 0.5F + textureOffset, 0.5F - textureOffset, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, 1.0F, 0.0F});

        part.setIndexes(new short[]{0, 2, 1, 0, 3, 2, 0, 1, 5, 0, 5, 4, 1, 2, 5, 2, 6, 5, 6, 3, 7, 2, 3, 6, 0, 4, 3, 3, 4, 7});
