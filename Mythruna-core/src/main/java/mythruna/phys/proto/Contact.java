...
public class Contact {
    ...
    private static final double VELOCITY_LIMIT = 0.1D;
    ...
    protected void matchAwakeState() {
        ...
        boolean awake2 = this.body2.isAwake();
    ...
    protected void applyPositionChange(Vec3d linear1, Vec3d linear2, Vec3d angular1, Vec3d angular2, double penetration) {
        ...
        if ((this.body1 != null) && (!this.body1.isStatic())) {
            ...
            Vec3d pos = this.body1.getPosition().clone();

            addScaledVector(pos, this.contactNormal, linearMove1);
            this.body1.setPosition(pos);

            if (angularMove1 != 0.0D) {
                Quatd q = this.body1.getOrientation();

                addScaledVector(q, angular1, 1.0D);
        ...
        if ((this.body2 != null) && (!this.body2.isStatic())) {
            ...
            addScaledVector(projection, this.contactNormal, -this.relativeContactPosition2.dot(this.contactNormal));
            ...
            Vec3d pos = this.body2.getPosition().clone();

            addScaledVector(pos, this.contactNormal, linearMove2);
            this.body2.setPosition(pos);

            Quatd q = this.body2.getOrientation();
            addScaledVector(q, angular2, 1.0D);
