...
public class TowerGenerator implements ChunkGenerator {

    int size;

    public TowerGenerator(int size) {
        this.size = size;
    }

    public void generateNodes(Level l, int x, int y, SneakAroundTown app) {
        Box b = new Box(l.chunkSize / 2, 10 + (5 * size), l.chunkSize / 2);
        ...
        phy.setPhysicsLocation(new Vector3f(x * l.chunkSize + l.chunkSize * .5f, 10 + (5 * size), y * l.chunkSize + l.chunkSize * .5f));
    }
}
