package com.pro.game.example.petomancer;
...
public class DungeonRenderer {
    ...
    public static void onInit(SimpleApplication scene) {
        ...
        Material fullTile = scene.getAssetManager().loadMaterial("assets/Materials/CobbleStoneGround.j3m");
        Material wall = scene.getAssetManager().loadMaterial("assets/Materials/CobbleStoneGround.j3m");
    ...
    public static void setPlayerStartPosition(Map map, SimpleApplication scene) {
        ...
        for (int i = 0; i < map.getWidth(); i++) {
            for (int j = 0; j < map.getHeight(); j++) {
                if (!(map.getTile(i, j) == 0) && (i <= playerStartLocation.x) && (j <= playerStartLocation.z))
                    playerStartLocation = new Vector3f(i + 0.5f, 0, j + 0.5f);
            }
        }
